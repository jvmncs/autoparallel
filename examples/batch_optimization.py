...
def compare_objectives_for_model():
    ...
    print(
        f"{'Objective':<18} {'GPUs':<6} {'Memory Use':<12} {'Config':<25} {'Score':<8}"
    )
    print("-" * 75)
    ...
    for objective in objectives:
        try:
            ...
            print(
                f"{objective:<18} {config['total_gpus']:<6} {config['memory_utilization']:<11.1%} {config_str:<25} {config['score']:<8.2f}"
            )
...
def demonstrate_cost_estimation():
    ...
    print(f"Model: {model}")
    print(f"Cluster: {cluster['gpu_count']}x {cluster['gpu_memory_gb']}GB GPUs")
    print("Assumptions: 730 hours/month at $3/GPU/hour")
    print()

    print(
        f"{'Objective':<18} {'GPUs':<6} {'$/Hour':<10} {'$/Month':<12} {'Memory %':<10}"
    )
    print("-" * 60)
    ...
    for objective, analysis in cost_analysis["cost_analysis"].items():
        if "error" not in analysis:
            print(
                f"{objective:<18} {analysis['gpus_used']:<6} ${analysis['cost_per_hour']:<9.0f} ${analysis['cost_per_month']:<11.0f} {analysis['memory_utilization']:<9.1%}"
            )
        else:
...
def analyze_scaling_scenarios():
    ...
    for scenario in scenarios:
        ...
        print(
            f"Cluster: {scenario['cluster']['gpu_count']}x {scenario['cluster']['gpu_memory_gb']}GB"
        )
        ...
        try:
            ...
            print(
                f"Result: Uses {config['total_gpus']} GPUs ({config['memory_utilization']:.1%} memory)"
            )
            print(
                f"Config: TP={config['tensor_parallel']}, DP={config['data_parallel']}"
            )
...
def demonstrate_trade_off_analysis():
    ...
    for obj, config in results.items():
        ...
        print(
            f"  {obj:<18}: {config['total_gpus']}/{cluster['gpu_count']} GPUs ({efficiency:.1%})"
        )
    ...
    print(
        "• maximize_throughput: Choose for production inference where performance matters"
    )
...
def demonstrate_memory_requirements():
    ...
    memory_req = autoparallel.check_memory_requirements(
        model=model, sequence_length=2048, batch_size=1, quantization="fp16"
    )

    print(f"Model: {model}")
    print(f"Total memory required: {memory_req['total_memory_gb']:.2f}GB")
    print(
        f"Recommended GPU memory: {memory_req['single_gpu_requirements']['recommended_memory_gb']:.1f}GB"
    )
    print()
...
def demonstrate_real_world_scenarios():
    ...
    try:
        config = autoparallel.best_config(
            model="microsoft/DialoGPT-small", cluster=cluster, objective="minimize_gpus"
        )
        print(
            f"• Result: {config['total_gpus']} GPUs, {config['memory_utilization']:.1%} memory use"
        )
    except Exception as e:
        print(f"• Error: {e}")
    ...
    try:
        config = autoparallel.best_config(
            model="microsoft/DialoGPT-small", cluster=cluster, objective="balance"
        )
        print(
            f"• Result: {config['total_gpus']} GPUs, {config['memory_utilization']:.1%} memory use"
        )
...
def main():
    ...
    try:
        ...
        print("🎯 minimize_gpus:")
        print(
            "   • Use when: Limited GPU budget, development/testing, resource constraints"
        )
        ...
        print(
            "   • Use when: Production inference, high request volume, performance critical"
        )
        ...
        print()
        print(
            "💡 Pro tip: Start with 'balance' objective, then specialize based on needs"
        )
